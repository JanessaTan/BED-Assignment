document.addEventListener("DOMContentLoaded", function initialiseStalls() {
  ...
  function render() {
    ...
    let stalls = allStalls.filter((stall) => {
      ...
      return matchesQuery && (!cuisine || stall.cuisine === cuisine) && (!grade || stall.hygiene === grade);
    });
    stalls.sort((a, b) => {
    ...
    document.getElementById("stallResults").innerHTML = stalls.map((stall) => {
      ...
      return `
        <article class="card stall-card">
          <div class="row-between"><span class="badge badge-primary">${HC.escapeHtml(stall.cuisine)}</span><span class="badge ${HC.hygieneBadgeClass(stall.hygiene)}">${HC.hygieneText(stall.hygiene)}</span></div>
