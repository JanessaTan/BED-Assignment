document.addEventListener("DOMContentLoaded", function initialiseSalesAnalytics() {
  ...
  function render() {
    ...
    document.getElementById("analyticsStats").innerHTML = [
      ["Total sales", HC.formatCurrency(totalSales)],
      ["Number of orders", orders.length],
      ["Average order value", HC.formatCurrency(average)]
    ...
    document.getElementById("analyticsPopularItems").innerHTML = popular.map(([name, count]) => `<div class="popular-bar"><strong>${HC.escapeHtml(name)}</strong><div class="progress"><span style="width:${(count / maxCount) * 100}%"></span></div><span>${count} sold</span></div>`).join("");
