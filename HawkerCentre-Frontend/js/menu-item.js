document.addEventListener("DOMContentLoaded", async () => {
  ...
  // Retrieve the selected stall and its menu items from the backend.
  async function loadPage() {
    ...
    try {
      ...
      if (!stall) {
        throw new Error("The selected stall was not found.");
      }

      document.getElementById("backToStalls").href =
        `stalls.html?centre=${stall.centreId}`;
      document.getElementById("stallName").textContent = stall.name;
      document.getElementById("stallDescription").textContent =
      ...
      document.getElementById("stallMeta").innerHTML = [
        ...
        .filter(Boolean)
        .map((value) => `<span>${HC.escapeHtml(value)}</span>`)
        .join("");
      document.getElementById("stallBadges").innerHTML = `
        <span class="badge ${HC.hygieneBadgeClass(stall.hygieneGrade)}">${HC.escapeHtml(
          stall.hygieneGrade
      ...
      document.getElementById("categoryFilter").insertAdjacentHTML(
        "beforeend",
        categories
          .map((category) => `<option>${HC.escapeHtml(category)}</option>`)
          .join("")
      );
  ...
  // Convert the backend response into the format expected by the cart.
    ...
    results.innerHTML = items
      .map((item) => {
        const addOns = item.addOns.length
          ? `<fieldset class="menu-addons"><legend>Optional add-ons</legend>${item.addOns
              .map(
                (addOn, index) =>
                  `<label><input type="checkbox" data-addon-index="${index}"> ${HC.escapeHtml(addOn.name)} (+${HC.formatCurrency(addOn.price)})</label>`
              )
              .join("<br>")}</fieldset>`
        ...
        return `
          <article class="card menu-card" data-menu-card="${item.id}">
            ...
            <div class="card-meta">
              <span>${(item.cuisines || []).map(HC.escapeHtml).join(" · ")}</span>
              <span>About ${item.prep} min</span>
  ...
  function handleMenuAction(event) {
    ...
    const item = allItems.find(
      (candidate) => candidate.id === card.dataset.menuCard
    );
    if (!item) return;
    ...
    if (event.target.matches("[data-add-cart]") && item.available) {
      ...
      HC.addToCart(
        item,
        Number(quantityOutput.value || quantityOutput.textContent),
        addOns
      );
